    ...
    (check-error (find-formula-true-asst 'x (lambda () 'fail) (lambda ()
                                                                     'succeed)))
    ...
    (check-expect   ; (make-and (list2 (make-not 'x) 'x)) cannot be solved
       (find-formula-true-asst (make-and (list2 (make-not 'x) 'x))
                               (lambda () 'fail)
                               (lambda (cur resume) (find 'x cur)))
       'fail)
