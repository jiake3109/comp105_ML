...
(* Function freeIn is of 'exp -> name -> bool', it takes a μScheme expression *)
(* and a name (a.k.a. a string) and tells whether that name is used for a free*)
(* variable in the expression                                                 *)
fun freeIn e name =
  let fun name_list bs = foldr (fn (x, xs) => (fst x) :: xs) [] bs
      fun exp_list  bs = foldr (fn (x, xs) => (snd x) :: xs) [] bs
      fun free (LITERAL v) = []
        | free (VAR x) = [x]
        | free (SET (x, e)) = x :: (free e)
        | free (IFX (e1, e2, e3)) = (free e1) @ (free e2) @ (free e3)
        | free (WHILEX (guard, body)) = (free guard) @ (free body)
        | free (BEGIN es) = foldr (fn (exp, exps) => (free exp) @ exps) [] es
        | free (LAMBDA (xs, e)) =
             let fun not_xs x = (member x xs) = false
             in  List.filter not_xs (free e)
             end
        | free (APPLY (f, args)) =
             let fun free_args (exp, exps) = (free exp) @ exps
             in  (free f) @ (foldr free_args [] args)
             end
        | free (LETX (LET, bs, body)) =
             let fun free_bs (exp, exps) = (free exp) @ exps
                 val free_exp            = foldr free_bs [] (exp_list bs)
                 fun not_bs x            = (member x (name_list bs)) = false
             in  free_exp @ (List.filter not_bs (free body))
             end
        | free (LETX (LETSTAR, bs, body)) =
             (case bs
               of []      => free body
                | (b::bs) => free (LETX (LET, [b], LETX (LETSTAR, bs, body))))
        | free (LETX (LETREC, bs, body)) =
             let fun free_bs (exp, exps) = (free exp) @ exps
                 val free_exp            = foldr free_bs [] (exp_list bs)
                 fun not_bs x            = (member x (name_list bs)) = false
             in  List.filter not_bs (free_exp @ (free body))
             end
  in  List.exists (fn x => x = name) (free e)
  end

(* Function improve is (name list * exp) * 'a env->(name list * exp) * 'a env *)
(* it takes a pair consisting of a LAMBDA’s components and an environment,    *)
(* and returns a better pair containing the same LAMBDA’s components paired   *)
(* with an environment that contains only the free variables of the LAMBDA    *)

fun improve ((xs, e), rho) =
        ((xs, e), List.filter (fn (x) => freeIn (LAMBDA (xs, e)) (fst x)) rho)

(* Unit Tests module *)
local
    val lit_1  = LITERAL (NUM 1)
    val lit_2  = LITERAL (SYM "x")
    val var_x  = VAR "x"
    val var_y  = VAR "y"
    val set_x1 = SET ("x", LITERAL (NUM 1))
    val set_y = SET ("y", var_x)
    val if_x = IFX (APPLY (VAR "=", [VAR "x", LITERAL (NUM 1)]), set_x1, set_y)
    val while_x = WHILEX (APPLY (VAR "=", [VAR "x", LITERAL (NUM 1)]), set_y)
    val begin_x = BEGIN [lit_1, var_x, set_x1, if_x, while_x]
    val apply_x = APPLY (VAR "map",
                        [LAMBDA (["x"], APPLY (VAR "=",
                                              [VAR "x", LITERAL (NUM 1)])),
                         VAR "xs"])
    val lambda_x = LAMBDA (["x"], LAMBDA (["y"], APPLY (VAR "equal?",
                                                       [VAR "x", VAR "y"])))
    val lambda_y = LAMBDA (["y"], APPLY (VAR "equal?", [VAR "x", VAR "y"]))
    val let_x = LETX (LET, [("x", var_x), ("y", var_y)], set_x1)
    val let_y = LETX (LET, [("x", var_y), ("y", var_y)], set_x1)
    val letrec_x = LETX (LETREC, [("x", var_x), ("y", var_y)], set_y)
    val letrec_y = LETX (LETREC, [("x", var_x), ("z", var_y)], set_y)
    val letstar_y = LETX (LETSTAR, [("x", var_y), ("y", var_y)], set_x1)
    val letstar_x = LETX (LETSTAR, [], set_x1)
    (* More local definitions can go here for unit testing... *)
in
        val () =
          Unit.checkAssert  "freeIn (literal num) false"
          (fn () => not (freeIn lit_1 "1"))

        val () =
          Unit.checkAssert  "freeIn (literal sym) false"
          (fn () => not (freeIn lit_2 "x"))

        val () =
          Unit.checkAssert  "freeIn (var name) true"
          (fn () => freeIn var_x "x")

        val () =
          Unit.checkAssert  "freeIn (var name) false"
          (fn () => not (freeIn var_x "y"))
        ...
        val () =
          Unit.checkAssert  "freeIn (apply_x) true"
          (fn () => freeIn apply_x "map")

        val () =
          Unit.checkAssert  "freeIn (apply_x) false"
          (fn () => not (freeIn apply_x "x"))

        val () =
          Unit.checkAssert  "freeIn (lambda_x) true"
          (fn () => freeIn lambda_x "equal?")

        val () =
          Unit.checkAssert  "freeIn (lambda_x) false"
        ...
        val () =
          Unit.checkAssert  "freeIn (lambda_y) false"
          (fn () => not (freeIn lambda_y "y"))

        val () =
          Unit.checkAssert  "freeIn (if_x) true"
          (fn () => freeIn if_x "y")

        val () =
          Unit.checkAssert  "freeIn (if_x) false"
          (fn () => not (freeIn if_x "1"))

        val () =
          Unit.checkAssert  "freeIn (while_x) true"
          (fn () => freeIn while_x "x")

        val () =
          Unit.checkAssert  "freeIn (while_x) false"
          (fn () => not (freeIn while_x "z"))

        val () =
          Unit.checkAssert  "freeIn (begin_x) true"
          (fn () => freeIn begin_x "=")

        val () =
          Unit.checkAssert  "freeIn (begin_x) false"
          (fn () => not (freeIn begin_x "z"))

        val () =
          Unit.checkAssert  "freeIn (let_x) true"
          (fn () => freeIn let_x "x")

        val () =
          Unit.checkAssert  "freeIn (let_y) false"
          (fn () => not (freeIn let_y "x"))

        val () =
          Unit.checkAssert  "freeIn (letrec_y) true"
          (fn () => freeIn letrec_y "y")

        val () =
          Unit.checkAssert  "freeIn (letrec_x) false"
          (fn () => not (freeIn letrec_x "x"))

        val () =
          Unit.checkAssert  "freeIn (letstar_x) true"
          (fn () => freeIn letstar_x "x")

        val () =
          Unit.checkAssert  "freeIn (letstar_y) false"
          (fn () => not (freeIn letstar_y "x"))
